...
// writer wins). The tmux toggle is unaffected since it only wraps the
// per-invocation SSH command.
func TestWorkspaceSSHControlsForCoder(t *testing.T) {
	...
	if !got.ShowTmux {
		t.Fatalf("Coder should still expose the tmux toggle, got ShowTmux=false")
	}
...
// both per-workspace toggles — each GitHub codespace has its own
...
func TestWorkspaceSSHControlsForGitHub(t *testing.T) {
	...
	if !got.ShowTmux {
		t.Fatalf("GitHub should expose tmux toggle, got ShowTmux=false")
	}
	if got.SharedConfNote != "" {
