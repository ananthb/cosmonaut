...
| Field | Type | Description |
|---|---|---|
| `hotkey` | string | Global hotkey (default `Cmd+Shift+S` on macOS, `Ctrl+Shift+S` elsewhere). Launches the default target, or opens the picker if no `defaultTarget` is set. |
